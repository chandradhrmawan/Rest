...
class PrintAndExport
{
  ...
  {
    $data = DB::connection("omcargo_ilcs")
      ->table("TX_HDR_TCA")
    ...
    if ($request == "DEL") {
      ...
      $req     = DB::connection('omcargo_ilcs')->table("TX_HDR_BM")->where('BM_NO', $data[0]->tca_req_no)->get();
      $hdrid   = $req[0]->bm_id;
      $req_det = DB::connection('omcargo_ilcs')->table("TX_DTL_BM")->where('HDR_BM_ID', $hdrid)->get();
  ...
  {
    $connect    = DB::connection("omcargo_ilcs");
    $header     = $connect->table("TX_HDR_UPER")->where('UPER_NO', $id)->get();
    $branch     = DB::connection('mdm_ilcs')->table("TM_BRANCH")->where('BRANCH_ID', $header[0]->uper_branch_id)->where('BRANCH_CODE', $header[0]->uper_branch_code)->get();
    ...
    $data       = DB::connection('omcargo_ilcs')->select($query);
  ...
  public static function printProformaNPK($id)
  {
    ...
    foreach ($detail as $list) {
      ...
      $componen  = DB::connection("mdm_ilcs")
        ->table("TM_COMP_NOTA")
    ...
    if (!array_key_exists("handling", $all)) {
      ...
      for ($i = 0; $i < count($bl); $i++) {
        $data       = DB::connection('omcargo_ilcs')->table("V_TX_DTL_NOTA")
    ...
    $query = "
            ...
            WHERE
              A.NOTA_ID = '$id'
            ";
    $kapal       = DB::connection('omcargo_ilcs')->select($query);
    $nota        = DB::connection('mdm_ilcs')->table('TM_NOTA')->where('NOTA_ID', $all['header'][0]->nota_group_id)->get();
    ...
    $uper        = DB::connection('omcargo_ilcs')->select($query);
    ...
    $sign        = DB::connection('mdm_ilcs')->table("TM_SIGNATURE")->where('SIGN_TYPE', "3")->where('SIGN_BRANCH_ID', $branch[0]->branch_id)->where('SIGN_BRANCH_CODE', $branch[0]->branch_code)->get();
  ...
  public static function printUperNPK($id)
  {
    ...
    if (!array_key_exists("handling", $all)) {
      ...
      for ($i = 0; $i < count($bl); $i++) {
        $data       = DB::connection('omcargo_ilcs')->table("V_TX_DTL_UPER")
    ...
    $nota       = DB::connection('mdm_ilcs')->table('TM_NOTA')->where('NOTA_ID', $all['header'][0]->uper_nota_id)->get();
  ...
  {
    $all               = [];
    $header            = DB::connection('omcargo_ilcs')->table('TX_HDR_BPRP')->where("BPRP_NO", "=", $id)->get();
    $detail            = DB::connection('omcargo_ilcs')->table('TX_DTL_BPRP')->where("HDR_BPRP_ID", "=", $header[0]->bprp_id)->get();
    $all["header"]     = $header;
    $all["detail"]     = $detail;
    $querya            = "
                          SELECT
    ...
    $queryb            = "
                         ...
                         A.BPRP_NO = '$id'
                        ";
    $a                 = DB::connection('omcargo_ilcs')->select($querya);
    $b                 = DB::connection('omcargo_ilcs')->select($queryb);
    ...
    if (!empty($a[0]->rec_id)) {
      ...
      $all["request"]  = $a;
      $branch          = DB::connection('mdm_ilcs')->table("TM_BRANCH")->where('BRANCH_ID', $a[0]->req_branch_id)->where('BRANCH_CODE', $a[0]->req_branch_code)->get();
      $sign            = DB::connection('mdm_ilcs')->table("TM_SIGNATURE")->where('SIGN_TYPE', "1")->where('SIGN_BRANCH_ID', $a[0]->req_branch_id)->where('SIGN_BRANCH_CODE', $a[0]->req_branch_code)->get();
      ...
      $all["request"]  = $b;
      $branch          = DB::connection('mdm_ilcs')->table("TM_BRANCH")->where('BRANCH_ID', $b[0]->req_branch_id)->where('BRANCH_CODE', $b[0]->req_branch_code)->get();
      $sign            = DB::connection('mdm_ilcs')->table("TM_SIGNATURE")->where('SIGN_TYPE', "1")->where('SIGN_BRANCH_ID', $a[0]->req_branch_id)->where('SIGN_BRANCH_CODE', $a[0]->req_branch_code)->get();
      $html            = view('print.bprp', ["sign" => $sign, "branch" => $branch, "header" => $all["header"], "detail" => $all["detail"], "request" => $all["request"]]);
    }
  ...
  public static function printRealisasiNPK($id)
  {
    ...
    $data["detail"] = $det;
    $branch         = DB::connection('mdm_ilcs')->table("TM_BRANCH")->where('BRANCH_ID', $header[0]->bm_branch_id)->where('BRANCH_CODE', $header[0]->bm_branch_code)->get();
  ...
  public static function printInvoiceNPK($id)
  {
    ...
    $all = ["header" => $header] + $det;
    $branch      = DB::connection('mdm_ilcs')->table("TM_BRANCH")->where('BRANCH_ID', $header[0]->nota_branch_id)->where('BRANCH_CODE', $header[0]->nota_branch_code)->get();
    ...
    if (!array_key_exists("handling", $all)) {
      ...
      for ($i = 0; $i < count($bl); $i++) {
        $data       = DB::connection('omcargo_ilcs')->table("TX_DTL_NOTA")
          ->where([['NOTA_HDR_ID ', '=', $id], ["dtl_bl", "=", $bl[$i]], ["dtl_group_tariff_id", "!=", "10"]])
          ->get();
        $handling[$bl[$i]] = json_decode(json_encode($data), TRUE);
      }
    ...
    foreach ($handa as $list) {
      ...
      $componena  = DB::connection("mdm_ilcs")
        ->table("TM_COMP_NOTA")
        ->where([['GROUP_TARIFF_ID', '=', $list->dtl_group_tariff_id], ["NOTA_ID", "=", $header[0]->nota_group_id], ["BRANCH_CODE", "=", $header[0]->nota_branch_code]])
        ->get();
    ...
    $sign        = DB::connection('mdm_ilcs')->table("TM_SIGNATURE")->where('SIGN_TYPE', "4")->where('SIGN_BRANCH_ID', $branch[0]->branch_id)->where('SIGN_BRANCH_CODE', $branch[0]->branch_code)->get();
  ...
  public static function printInvoiceNPKS($id)
  {
    ...
    // Data Uper And Payment
    $payment     = DB::connection('omuster_ilcs')->table("TX_PAYMENT")->where('PAY_REQ_NO', $header[0]->nota_req_no)->first();
    $uper        = $payment->pay_amount;
    $notaAmount  = $header[0]->nota_amount + $e_materai;
    $payAmount   = $uper + $e_materai;
    $total       = $notaAmount - $payAmount;
  ...
  public static function ExportTrafikProduksi($a, $b, $c, $d)
  {
    ...
    return view('print.trafik', ["data" => $result, "start" => $startDate, "end" => $endDate]);
  }
