...
use App\Models\OmUster\TmMenu_ilcs;
use App\Models\OmUster\TrRole_ilcs;
use App\Models\OmUster\TmUser_ilcs;
...
class UserAndRoleManagemnt{
  ...
  public static function storeUser($input){
    if (empty($input['user_id'])){
      $cek = DB::connection('omuster_ilcs')->table('TM_USER')->where('user_name',$input['user_name'])->count();
      if ($cek > 0) {
        return [
          "Success" => false,
      ...
      $cek = DB::connection('omuster_ilcs')->table('TM_USER')->where('user_nik',$input['user_nik'])->count();
      if ($cek > 0) {
        return [
          "Success" => false,
    ...
    if (empty($input['user_id'])) {
      ...
      DB::connection('omuster_ilcs')->table('TS_ROLE_BRANCH')->insert($TS_ROLE_BRANCH);
    }else{
      $set_data = [
      ...
      DB::connection('omuster_ilcs')->table('TM_USER')->where('user_id',$input['user_id'])->update($set_data);
      $TS_ROLE_BRANCH['user_id'] = $input['user_id'];
      $cek = DB::connection('omuster_ilcs')->table('TS_ROLE_BRANCH')->where('user_id', $input['user_id'])->where('branch_id', $input['user_branch_id'])->where('branch_code', $input['user_branch_code'])->where('role_id', $input['user_role'])->count();
      if ($cek == 0) {
        DB::connection('omuster_ilcs')->table('TS_ROLE_BRANCH')->insert($TS_ROLE_BRANCH);
      }
    }
  ...
  public static function changePasswordUser($input){
    DB::connection('omuster_ilcs')->table('TM_USER')->where('user_id',$input['user_id'])->update([
  ...
  public static function storeRolePermesion($input){
    DB::connection('omuster_ilcs')->table('TS_ROLE_MENU')->where('ROLE_ID', $input['ROLE_ID'])->delete();
    foreach ($input['MENU_ID'] as $id) {
      DB::connection('omuster_ilcs')->table('TS_ROLE_MENU')->insert([
  ...
  private static function role_data($role_id, $type = null){
    $role = DB::connection('omuster_ilcs')->table('TR_ROLE')->where('ROLE_ID', $role_id)->first();
    if ($type == 'role') {
      return $role;
    }else{
      if (empty($role)) {
  ...
  private static function role_p($role_id, $menu_id){
    ...
    $permi = DB::connection('omuster_ilcs')->table('TS_ROLE_MENU')->where('role_id',$role_id)->where('menu_id',$menu_id)->first();
    if (empty($permi)) {
      return false;
    }else{
      return true;
  ...
  public static function menuTree($roll_id){
    $menu_id_allow = DB::connection('omuster_ilcs')->table('TS_ROLE_MENU')->where('ROLE_ID', $roll_id)->orderBy('menu_id', 'asc')->pluck('menu_id');
  ...
  {
    $cek = DB::connection('omuster_ilcs')->table('TS_ROLE_BRANCH')->where('USER_ID',$input["USER_ID"])->where('ROLE_ID',$input["ROLE_ID"])->where('BRANCH_ID',$input["BRANCH_ID"])->where('BRANCH_CODE',$input["BRANCH_CODE"])->count();
    if ($cek > 0) {
      return [
        "Success" => false,
    ...
    DB::connection('omuster_ilcs')->table('TS_ROLE_BRANCH')->insert($store);
    return [
  ...
  public static function listRoleBranch($input){
    $rb = DB::connection('omuster_ilcs')->table('TS_ROLE_BRANCH');
    ...
    foreach ($rb as $rbl) {
      ...
      $user = DB::connection('omuster_ilcs')->table('TM_USER')->select('user_name','user_full_name','user_nik')->where('user_id', $rbl->user_id)->take(1)->get();
      foreach ($user as $user) {
        foreach ($user as $key => $value) {
          $newDt[$key] = $value;
        }
      }
      $role = DB::connection('omuster_ilcs')->table('TR_ROLE')->leftJoin('TM_REFF', 'role_service', '=', 'reff_id')->select('role_name', 'reff_name as role_service_name')->where('role_id', $rbl->role_id)->where('reff_tr_id', 1)->take(1)->get();
      foreach ($role as $role) {
        foreach ($role as $key => $value) {
          $newDt[$key] = $value;
        }
      }
      $branch = DB::connection('mdm_ilcs')->table('TM_BRANCH')->where('branch_id', $rbl->branch_id)->where('branch_code', $rbl->branch_code)->take(1)->get();
  ...
  {
    DB::connection('omuster_ilcs')->table('TS_ROLE_BRANCH')->where('USER_ID',$input["USER_ID"])->where('ROLE_ID',$input["ROLE_ID"])->where('BRANCH_ID',$input["BRANCH_ID"])->where('BRANCH_CODE',$input["BRANCH_CODE"])->delete();
