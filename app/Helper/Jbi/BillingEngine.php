...
class BillingEngine {
  ...
  public static function storeProfileTariff($input){
        ...
        // store head
          if(empty($head['TARIFF_ID'])){
            $headS    = new TxProfileTariffHdr_ilcs;
          }else{
            $headS    = TxProfileTariffHdr_ilcs::find($head['TARIFF_ID']);
          }
        ...
        if (!empty($head['FILE']['PATH'])) {
          ...
          if ($response['response'] == true) {
            TxProfileTariffHdr_ilcs::where('tariff_id',$headS->tariff_id)->update([
        ...
        // store detil
          if (count($input['detil']) > 0) {
            TsTariff_ilcs::where('tariff_prof_hdr_id', $headS->tariff_id)->delete();
          }
          foreach ($input['detil'] as $list) {
              ...
              if (!empty($list['ALAT'])) {
                ...
                $query .= "'') ISO FROM dual";
                $alatisocode = \DB::connection('mdm_ilcs')->select(DB::raw($query));
              ...
              if (!empty($list['BARANG'])) {
                ...
                $query .= ") ISO FROM dual";
                $itemisocode = \DB::connection('mdm_ilcs')->select(DB::raw($query));
              ...
              if (isset($list['tariff_id']) and !empty($list['tariff_id'])) {
                $detilS                     = TsTariff_ilcs::find($list['tariff_id']);
              }else{
                $detilS                     = new TsTariff_ilcs;
              }
        ...
        $listHeader = TxProfileTariffHdr_ilcs::find($headS->tariff_id);
  ...
  public static function deleteProfileTariffDetil($input){
    TsTariff_ilcs::where('tariff_id', $input['id'])->delete();
    return [ "result" => "Success, delete profile tariff detil data" ];
  }

  public static function storeCustomerProfileTariffAndUper($input){
        DB::connection('eng_ilcs')->table('TS_CUSTOMER_PROFILE')->where('CUST_PROFILE_ID', $input['CUST_PROFILE_ID'])->delete();
        foreach ($input['TARIFF'] as $list) {
          DB::connection('eng_ilcs')->table('TS_CUSTOMER_PROFILE')->insert([
        ...
        DB::connection('eng_ilcs')->table('TS_UPER')->where('UPER_CUST_ID', $input['CUST_PROFILE_ID'])->delete();
        foreach ($input['UPER'] as $list) {
          DB::connection('eng_ilcs')->table('TS_UPER')->insert([
  ...
  public function listProfileTariffDetil($input){
    $detil = DB::connection('eng_ilcs')->table('TS_TARIFF')->where('TARIFF_PROF_HDR_ID', $input['tariff_id']);
    $order_key = 'tariff_id';
    ...
    foreach ($detil as $list) {
      ...
      if (!empty($list->tariff_reference)) {
        $name_tariff_reference = DB::connection('eng_ilcs')->table('TM_REFF')->where('reff_id', $list->tariff_reference)->where('reff_tr_id', 2)->get();
      ...
      foreach ($list as $key => $value) {
        ...
        if (strtoupper($key) == 'ISO_CODE' and !empty($value)) {
          ...
          $cont_status_desc = "";
          $cont_type_desc = "";
          $equi = DB::connection('mdm_ilcs')->table('TM_ISO_EQUIPMENT')->where('ISO_CODE',$value)->first();
          if (!empty($equi)) {
            foreach ($equi as $keyS => $valueS) {
              ...
              if (strtoupper($keyS) == 'EQUIPMENT_TYPE_ID') {
                $equipment_type_name = DB::connection('mdm_ilcs')->table('TM_EQUIPMENT_TYPE')->where('EQUIPMENT_TYPE_ID',$valueS)->first()->equipment_type_name;
              } else if (strtoupper($keyS) == 'EQUIPMENT_UNIT') {
                $get = DB::connection('mdm_ilcs')->table('TM_UNIT')->where('UNIT_ID',$valueS)->first();
          ...
          $cont = DB::connection('mdm_ilcs')->table('TM_ISO_CONT')->where('ISO_CODE',$value)->first();
          if (!empty($cont)) {
            foreach ($cont as $keyS => $valueS) {
              ...
              if (strtoupper($keyS) == 'CONT_SIZE' and !empty($valueS)) {
                $get = DB::connection('mdm_ilcs')->table('TM_CONT_SIZE')->where('CONT_SIZE',$valueS)->first();
                $cont_desc = $get->cont_desc;
              }
              if (strtoupper($keyS) == 'CONT_STATUS' and !empty($valueS)) {
                $get = DB::connection('mdm_ilcs')->table('TM_CONT_STATUS')->where('CONT_STATUS',$valueS)->first();
                $cont_status_desc = $get->cont_status_desc;
              }
               if (strtoupper($keyS) == 'CONT_TYPE' and !empty($valueS)) {
                $get = DB::connection('mdm_ilcs')->table('TM_CONT_TYPE')->where('CONT_TYPE',$valueS)->first();
                $cont_type_desc = $get->cont_type_desc;
              }
            }
          }
          $como = DB::connection('mdm_ilcs')->table('TM_ISO_COMMODITY')->where('ISO_CODE',$value)->first();
          if (!empty($como)) {
            foreach ($como as $keyS => $valueS) {
              ...
              if (strtoupper($keyS) == 'PACKAGE_ID' and !empty($valueS)) {
                $get = DB::connection('mdm_ilcs')->table('TM_PACKAGE')->where('PACKAGE_ID',$valueS)->first();
                $package_name = $get->package_name;
                $package_code = $get->package_code;
              }
              if (strtoupper($keyS) == 'COMMODITY_ID' and !empty($valueS)) {
                $get = DB::connection('mdm_ilcs')->table('TM_COMMODITY')->where('COMMODITY_ID',$valueS)->first();
          ...
          $group_tariff_name = DB::connection('eng_ilcs')->table('TM_GROUP_TARIFF')->where('GROUP_TARIFF_ID',$value)->get()[0]->group_tarif_name;
        } else if(strtolower($key) == 'sub_tariff' and !empty($value)){
          $group_tariff_name = DB::connection('eng_ilcs')->table('TM_REFF')->where('REFF_TR_ID', 7)->where('REFF_ID',$value)->get()[0]->reff_name;
        } else if(strtolower($key) == 'nota_id' and !empty($value)){
          $nota = DB::connection('eng_ilcs')->table('TM_NOTA')->where('nota_id',$value)->first();
          if (!empty($nota)) {
      ...
      $stacking = DB::connection('eng_ilcs')->table('TM_REFF')->where([['reff_id','=', $list->stacking_area],['REFF_TR_ID','=', '11']])->select("reff_name as stacking_area_name")->get();
      if (empty($stacking)) {
  ...
  public static function viewProfileTariff($input){
    $header = TxProfileTariffHdr_ilcs::find($input['TARIFF_ID']);
    $detil = DB::connection('eng_ilcs')->table('TS_TARIFF')->where('TARIFF_PROF_HDR_ID', $input['TARIFF_ID'])->orderBy('TARIFF_ID', 'DESC')->get();
    $response_detil = [];
    foreach ($detil as $list) {
      $equipment_type_name = "";
      $equipment_unit_name = "";
      ...
      $newDt['tariff_reference_name'] = '';
      if (!empty($list->tariff_reference)) {
        $name_tariff_reference = DB::connection('mdm_ilcs')->table('TM_REFF')->where('reff_id', $list->tariff_reference)->where('reff_tr_id', 2)->get();
        if (count($name_tariff_reference) > 0) {
          $name_tariff_reference = $name_tariff_reference[0];
          $newDt['tariff_reference_name'] = $name_tariff_reference->reff_name;
        }
      ...
      foreach ($list as $key => $value) {
        ...
        if (strtoupper($key) == 'PLUGGIN_UNIT' and !empty($value)) {
          $plugging_unit_name = DB::connection('mdm_ilcs')->table('TM_UNIT')->where('unit_id', $value)->first();
          $plugging_unit_name = $plugging_unit_name->unit_name;
        }
        if (strtoupper($key) == 'VIA' and !empty($value)) {
          $via_name = DB::connection('mdm_ilcs')->table('TM_REFF')->where('reff_tr_id',4)->where('reff_id', $value)->first();
          $via_name = $via_name->reff_name;
        }
        if (strtoupper($key) == 'FUMIGASI_TYPE' and !empty($value)) {
          $fumigasi_name = DB::connection('mdm_ilcs')->table('TM_REFF')->where('reff_tr_id',12)->where('reff_id', $value)->first();
        ...
        if (strtoupper($key) == 'ISO_CODE' and !empty($value)) {
          ...
          $group_tariff_name = DB::connection('mdm_ilcs')->table('TM_GROUP_TARIFF')->where('GROUP_TARIFF_ID',$value)->get()[0]->group_tarif_name;
        } else if(strtolower($key) == 'sub_tariff' and !empty($value)){
          $group_tariff_name = DB::connection('mdm_ilcs')->table('TM_REFF')->where('REFF_TR_ID', 7)->where('REFF_ID',$value)->get()[0]->reff_name;
        } else if(strtolower($key) == 'nota_id' and !empty($value)){
          $nota = DB::connection('mdm_ilcs')->table('TM_NOTA')->where('nota_id',$value)->first();
          if (!empty($nota)) {
            foreach ($nota as $keyS => $valueS) {
              if (strtoupper($keyS) != 'SERVICE_CODE') {
                $newDt[$keyS] = $valueS;
      ...
      $stacking = DB::connection('mdm_ilcs')->table('TM_REFF')->where([['reff_id','=', $list->stacking_area],['REFF_TR_ID','=', '11']])->select("reff_name as stacking_area_name")->get();
      if (empty($stacking)) {
        $newDt['stacking_area_name'] = '';
      } else {
        foreach ($stacking as $listS) {
  ...
  public static function viewCustomerProfileTariff($input){
    $TsCustomerProfile = DB::connection('eng_ilcs')->table('TS_CUSTOMER_PROFILE')->leftJoin('TX_PROFILE_TARIFF_HDR', 'TS_CUSTOMER_PROFILE.TARIFF_HDR_ID', '=', 'TX_PROFILE_TARIFF_HDR.TARIFF_ID')->where('CUST_PROFILE_ID', $input['CUST_PROFILE_ID'])->get();
    $TsUper = DB::connection('eng_ilcs')->table('TS_UPER')->leftJoin('BILLING_mdm_ilcs.TM_NOTA', 'TS_UPER.UPER_NOTA', '=', 'TM_NOTA.NOTA_ID')->where('UPER_CUST_ID', $input['CUST_PROFILE_ID'])->get();
    return [
  ...
  public static function calculateTariff($input){
    // build detil
        ...
        $countD = 0;
        $setD = '';
        $getServCode = DB::connection('mdm_ilcs')->table('TM_NOTA')->where('nota_id',$input['head']['P_NOTA_ID'])->first();
        ...
        foreach ($detil as $list) {
          ...
          // if (!empty($list['DTL_RELOKASI_TYPE'])) {
          //   if ($list['DTL_RELOKASI_TYPE'] == NULL or $list['DTL_RELOKASI_TYPE'] == 'NULL') {
          //     $setD .= ' detail.DTL_RELOKASI_TYPE  := '.$list['DTL_RELOKASI_TYPE'].';';
          //   }else{
          //     $setD .= ' detail.DTL_RELOKASI_TYPE  := \''.$list['DTL_RELOKASI_TYPE'].'\';';
          //   }
          // }
  ...
  private static function calculateTariffExcute($input){
      $head = DB::connection('eng_ilcs')->table('TX_TEMP_TARIFF_HDR')->where('BOOKING_NUMBER', $input['b_no'])->get();

      if (!empty($head)) {
        $head = $head[0];
        DB::connection('eng_ilcs')->table('TX_LOG')->where('TEMP_HDR_ID', $head->temp_hdr_id)->delete();
        DB::connection('eng_ilcs')->table('TX_TEMP_TARIFF_DTL')->where('TEMP_HDR_ID', $head->temp_hdr_id)->delete();
        DB::connection('eng_ilcs')->table('TX_TEMP_TARIFF_SPLIT')->where('TEMP_HDR_ID', $head->temp_hdr_id)->delete();
        DB::connection('eng_ilcs')->table('TX_TEMP_TARIFF_HDR')->where('BOOKING_NUMBER', $input['b_no'])->delete();
      }
      $getConfLink = config('database.connections.eng_ilcs');
      $link = oci_connect($getConfLink['username'], $getConfLink['password'], $getConfLink['host'].':'.$getConfLink['port'].'/'.$getConfLink['database']);
      $getServCode = DB::connection('mdm_ilcs')->table('TM_NOTA')->where('nota_id',$input['inputH']['P_NOTA_ID'])->first();
      ...
      // gak nemu buat nerima retun pesan dari prosedur // di ubah cara peng_ilcsecekannya ngambil dari table TX_LOG
      ...
      if (empty($head)) {
        ...
        $head = $head[0];
        $head = (array)$head;
        $result = DB::connection('eng_ilcs')->table('TX_LOG')->where('TEMP_HDR_ID', $head['temp_hdr_id'])->get();
        $result = $result[0];
        $result = (array)$result;
        ...
        $cekMemoline = DB::connection('eng_ilcs')->table('TX_TEMP_TARIFF_DTL')->where('TEMP_HDR_ID', $head['temp_hdr_id'])->whereNull('memoline')->get();
  ...
  public static function getSimulasiTarif($input) {
    // build head
        $head                       = $input["HEADER"];
        $pbmCek = 'N';
        $countPBM = DB::connection('mdm_ilcs')->table('TM_PBM_INTERNAL')->where('PBM_ID',$head['P_PBM_ID'])->where('BRANCH_ID',$head['P_BRANCH_ID'])->where('BRANCH_CODE',$head['P_BRANCH_CODE'])->count();
    ...
    // set data
      ...
      $getHS = DB::connection('eng_ilcs')->table('V_PAY_SPLIT')->where('booking_number',$head['P_BOOKING_NUMBER'])->get();
      foreach ($getHS as $getH){
          ...
          foreach ($group_tariff as $grpTrf){
              $grpTrf = (array)$grpTrf;
              $uperD = DB::connection('eng_ilcs')->table('V_TX_TEMP_TARIFF_DTL')->where('TEMP_HDR_ID',$getH->temp_hdr_id)->where('group_tariff_id',$grpTrf['group_tariff_id'])->get();
              $countLine = 0;
