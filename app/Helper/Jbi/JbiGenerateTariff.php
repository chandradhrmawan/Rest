...
class JbiGenerateTariff
{
	...
	{
		$tglIn 	= DB::connection('omuster_ilcs')
			->table('TX_GATEIN')
	...
	private static function getLastContFromTX_HISTORY_CONTAINER($list, $hdr, $config, $input)
	{
		// check kegiatan
		/*
		   select tr.reff_id tr_reff_id, tr.reff_name tr_reff_name, tm.reff_id tm_reff_id, tm.reff_name tm_reff_name, tm.reff_order
		   from tm_reff tm
		   join tr_reff tr on tr.reff_id = tm.reff_tr_id
		   where tr.reff_id=12
		   order by tm.reff_order asc;
		 */
		...
		if (!in_array($input['nota_id'], [2, 16])) {
			...
			#add nota_id 1 untuk send req delivery, karna data awal dari reveiving ?
			// hapus kegiatan 1 fix by firman, sukses update history billing order setelah placement
	...
	private static function getDTL_RELOKASI_TYPE($config, $list, $hdr, $input)
	{

		if (empty($config['DTL_RELOKASI_TYPE'])) {
			$DTL_RELOKASI_TYPE = 'NULL';
		} else {
			//$DTL_RELOKASI_TYPE = empty($list[$config['DTL_FUMI_TYPE']]) ? 'NULL' : strtoupper($list[$config['DTL_FUMI_TYPE']]);
			$id_relokasi = empty($hdr['id_relokasi_type']) ? 'NULL' : $hdr['id_relokasi_type'];
			if (empty($id_relokasi)) {
				$$relokasi_type = 'NULL';
			} else {
				$relokasi = DB::connection('omuster_ilcs')->table('TM_RELOKASI_TYPE')->where('ID_RELOKASI_TYPE', $id_relokasi)->first();
				$relokasi_type = $relokasi->relokasi_type;
			}

			$DTL_RELOKASI_TYPE = $relokasi_type;
		}

		return $DTL_RELOKASI_TYPE;
	}
	...
	{
		$getPFS = DB::connection('mdm_ilcs')->table('TM_COMP_NOTA')->where('NOTA_ID', $input['nota_id'])->where('BRANCH_ID', $hdr[$config['head_branch']])->where('BRANCH_CODE', $hdr[$config['head_branch_code']])->where('GROUP_TARIFF_ID', 15)->count();
		if ($getPFS > 0) {
			$DTL_PFS = 'Y';
		} else {
			$DTL_PFS = 'N';
		...
		if (empty($config['DTL_DATE_IN'])) {
			...
			if (in_array($config['DTL_DATE_IN'], ["TX_GATEIN"])) {
				...
				if (!empty($config['DTL_STACK_DATE'])) {
					DB::connection('omuster_ilcs')->table($config['head_tab_detil'])->where($config['DTL_PRIMARY'], $list[$config['DTL_PRIMARY']])->update([$config['DTL_STACK_DATE'] => $dateIn]);
				}
				$DTL_DATE_IN = 'to_date(\'' . \Carbon\Carbon::parse($dateIn)->format('Y-m-d H:i:s') . '\',\'YYYY-MM-DD HH24:MI:SS\')';
	...
	private static function getDTL_DATE_OUT($config, $list, $hdr, $input)
	{
		if (empty($config['DTL_DATE_OUT'])) {
			...
			if ($config['head_table'] == 'TX_HDR_DEL_CARGO') {
				...
				// $tglOut = DB::connection('omuster_ilcs')->table('TX_GATEOUT')->orderBy("GATEOUT_DATE", "DESC")->first();
	...
	private static function getDTL_DATE_OUT_OLD($config, $list, $hdr)
	{
		if (!empty($config['head_ext_status']) and $hdr[$config['head_ext_status']] == 'Y') {
			$getOldIdHdr = DB::connection('omuster_ilcs')->table($config['head_table'])->where($config['head_no'], $hdr[$config['head_ext_from']])->first();
			$getOldIdHdr = (array) $getOldIdHdr;
			$getOldIdHdr = $getOldIdHdr[$config['head_primery']];
			$getOldDtDtl = DB::connection('omuster_ilcs')->table($config['head_tab_detil'])->where([
	...
	public static function calculateTariffBuild($find, $input, $config, $canceledReqPrepare)
	{
		...
		$setD = [];
		$detil = DB::connection('omuster_ilcs')->table($config['head_tab_detil'])->where($config['head_forigen'], $find[$config['head_primery']]);
	...
	private static function calculateTariffBuildDetail($hdr, $list, $input, $config)
	{
		...
		//$newD['DTL_RELOKASI_TYPE'] = static::getDTL_RELOKASI_TYPE($config,$list,$hdr,$input);
	...
	public static function showTempTariff($query, $config, $find)
	{
		...
		foreach ($getHS as $getH) {
			...
			foreach ($comp_notas as $comp_nota) {
				$grArr = DB::connection('mdm_ilcs')->table('TM_COMP_NOTA')->where([
	...
	public static function simulationTariffJBI($input)
	{
		$setH = [];
		// head
