...
use App\Models\OmCargo\TxPayment_ilcs;
use App\Models\OmCargo\TxHdrUper_ilcs;

class RealisasiHelper{

  public static function confirmRealBM($input){
    $find = DB::connection('omcargo_ilcs')->table('TX_HDR_REALISASI')->leftJoin('TX_HDR_BM', 'TX_HDR_REALISASI.REAL_REQ_NO', '=', 'TX_HDR_BM.BM_NO')->where('real_id',$input['id'])->get();
    if (empty($find)) {
      return ['Success' => false, 'result' => 'Fail, not found data!'];
    }
    $find = $find[0];
    $countPBM = DB::connection('mdm_ilcs')->table('TM_PBM_INTERNAL')->where('PBM_ID',$find->bm_pbm_id)->where('BRANCH_ID',$find->bm_branch_id)->where('BRANCH_CODE',$find->bm_branch_code)->count();
    ...
    // build eqpt
      $setE = [];
      $eqpt = DB::connection('omcargo_ilcs')->table('TX_EQUIPMENT')->where('req_no', $find->real_no)->get();
    ...
    // build detil
      $setD = [];
      // return $detil = DB::connection('omcargo_ilcs')->table('TX_DTL_REALISASI')->where('hdr_real_id', $find->real_id)->get();
      $detil = DB::connection('omcargo_ilcs')->table('TX_DTL_REALISASI')->leftJoin('TX_DTL_BM', 'TX_DTL_REALISASI.DTL_BM_ID', '=', 'TX_DTL_BM.DTL_BM_ID')->where('hdr_real_id', $find->real_id)->get();
      foreach ($detil as $list) {
        ...
        $newD['DTL_VIA'] = 'NULL';
        $getPFS = DB::connection('mdm_ilcs')->table('TM_COMP_NOTA')->where('NOTA_ID', 13)->where('BRANCH_ID',$find->bm_branch_id)->where('GROUP_TARIFF_ID', 15)->count();
        if ($getPFS > 0) {
          $newD['DTL_PFS'] = 'Y';
        }else{
          $newD['DTL_PFS'] = 'N';
    ...
    // build paysplit
      $setP = [];
      if ($find->bm_split == 'Y') {
        $paysplit = DB::connection('omcargo_ilcs')->table('TX_SPLIT_NOTA')->where('req_no', $find->real_req_no)->get();
    ...
    // return $tariffResp = Billingeng_ilcsine::calculateTariff($set_data);
    ...
    DB::connection('omcargo_ilcs')->table('TX_HDR_REALISASI')->where('real_id',$input['id'])->update([
      "real_status" => 2
    ]);
  ...
  public static function confirmRealBPRP($input){
    $find = DB::connection('omcargo_ilcs')->table('TX_HDR_BPRP')->where('bprp_id',$input['id'])->get();
    if (empty($find)) {
      return ['Success' => false, 'result' => 'Fail, not found data!'];
    }
    $find = $find[0];
    ...
    // build eqpt
      $setE = [];
      $eqpt = DB::connection('omcargo_ilcs')->table('TX_EQUIPMENT')->where('req_no', $find->bprp_no)->get();
      foreach ($eqpt as $list) {
        $newE = [];
        $list = (array)$list;
        $newE['EQ_TYPE'] = empty($list['eq_type_id']) ? 'NULL' : $list['eq_type_id'];
    ...
    // build detil
      $setD = [];
      $detil = DB::connection('omcargo_ilcs')->table('TX_DTL_BPRP')->where('hdr_bprp_id', $find->bprp_id)->get();
      foreach ($detil as $list) {
        $newD = [];
        $list = (array)$list;
        $newD['DTL_VIA'] = 'NULL';
        $getPFS = DB::connection('mdm_ilcs')->table('TM_COMP_NOTA')->where('NOTA_ID', $nota_id)->where('BRANCH_ID',$find->bprp_branch_id)->where('GROUP_TARIFF_ID', 15)->count();
        if ($getPFS > 0) {
          $newD['DTL_PFS'] = 'Y';
        }else{
          $newD['DTL_PFS'] = 'N';
    ...
    // build paysplit
      $setP = [];
      $cek_slpit = DB::connection('omcargo_ilcs')->select(DB::raw("SELECT DEL_SPLIT AS SPLIT_STATUS FROM TX_HDR_DEL WHERE DEL_NO = '".$find->bprp_req_no."' UNION ALL SELECT REC_SPLIT AS SPLIT_STATUS FROM TX_HDR_REC WHERE REC_NO = '".$find->bprp_req_no."'"));
      $cek_slpit = $cek_slpit[0];
      if ($cek_slpit->split_status == 'Y') {
        $paysplit = DB::connection('omcargo_ilcs')->table('TX_SPLIT_NOTA')->where('req_no', $find->bprp_req_no)->get();
        $paysplit = (array)$paysplit;
        foreach ($paysplit as $list) {
          $newP = [];
          $list = (array)$list;
    ...
    DB::connection('omcargo_ilcs')->table('TX_HDR_BPRP')->where('bprp_id',$input['id'])->update([
      "bprp_status" => 2
    ]);
  ...
  private static function migrateNotaData($booking_number,$req_no,$vessel_name,$ukk,$terminal_id, $tabReq, $reqNo){
    ...
    DB::connection('omcargo_ilcs')->table('TX_DTL_NOTA')->whereIn('nota_hdr_id',$hdr_id)->delete();
    ...
    foreach ($getHS as $getH) {
      // store head
        $app_id = DB::connection('omcargo_ilcs')->table($tabReq)->where($reqNo, $req_no)->get();
        $app_id = $app_id[0];
      ...
      $countLine = 0;
      DB::connection('omcargo_ilcs')->table('TX_DTL_NOTA')->where('nota_hdr_id', $headN->nota_id)->delete();
      foreach ($group_tariff as $grpTrf){
        $getD = DB::connection('eng_ilcs')->table('TX_TEMP_TARIFF_DTL')->where('TEMP_HDR_ID',$getH->temp_hdr_id)->where('group_tariff_id',$grpTrf->group_tariff_id)->get();
        foreach ($getD as $list) {
          $countLine++;
          DB::connection('omcargo_ilcs')->table('TX_DTL_NOTA')->insert([
  ...
  public static function rejectedProformaNota($input){
    ...
    if (!empty($input['file']) and !empty($input['file']['PATH']) and !empty($input['file']['BASE64'])) {
      ...
      $cekOldDoc  = DB::connection('omcargo_ilcs')->table('TX_DOCUMENT')->where('req_no', $input['proforma_no'])->get();
      if (count($cekOldDoc) > 0) {
        unlink($cekOldDoc[0]->doc_path);
        DB::connection('omcargo_ilcs')->table('TX_DOCUMENT')->where('req_no', $input['proforma_no'])->delete();
      }
      // Ubah
      $directory  = 'omcargo_ilcs/TX_DOCUMENT/'.date('d-m-Y').'/';
      $response   = FileUpload::upload_file($input['file'], $directory, "TX_NOTA_REJECT_PROFORMA", $input['req_no']);

      // $directory  = 'omcargo_ilcs/TX_DOCUMENT/rejectedProformaNota'.$input['req_no'].'/';
      // $response   = FileUpload::upload_file($input['file'], $directory);
      DB::connection('omcargo_ilcs')->table('TX_DOCUMENT')->insert([
    ...
    DB::connection('omcargo_ilcs')->table('TX_HDR_BPRP')->where('bprp_no',$input['req_no'])->update([
      "bprp_status"=>1
    ]);
    DB::connection('omcargo_ilcs')->table('TX_HDR_REALISASI')->where('real_no',$input['req_no'])->update([
      "real_status"=>1
    ]);
