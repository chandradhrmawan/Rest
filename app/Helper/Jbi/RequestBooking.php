...
class RequestBooking{
	// BTN
		public static function sendRequest($input){
			...
			$cekStatusNota = DB::connection('mdm_ilcs')->table('TS_NOTA')->where([
			...
			// build detil
				...
				foreach ($detil as $list) {
					...
					$getPFS = DB::connection('mdm_ilcs')->table('TM_COMP_NOTA')->where('NOTA_ID', $input['nota_id'])->where('BRANCH_ID',$find[$config['head_branch']])->where('BRANCH_CODE',$find[$config['head_branch_code']])->where('GROUP_TARIFF_ID', 15)->count();
					if ($getPFS > 0) {
						$newD['DTL_PFS'] = 'Y';
					}else{
						$newD['DTL_PFS'] = 'N';
	    ...
	    public static function approvalRequest($input){
			...
			$upers = DB::connection('eng_ilcs')->select(DB::raw($query));
			if (count($upers) == 0) {
				return ['result' => "Fail, uper and tariff not found!", "Success" => false];
			}
			$upPercent = DB::connection('eng_ilcs')->table('TS_UPER')->where('UPER_NOTA', $config['head_nota_id'])->where('BRANCH_ID', $find[$config['head_branch']])->where('UPER_CUST_ID', $find[$config['head_cust']])->get();
			if (count($upPercent) == 0) {
				$migrateTariff = false;
				$upPercent = DB::connection('eng_ilcs')->table('TS_UPER')->where('UPER_NOTA', $config['head_nota_id'])->where('BRANCH_ID', $find[$config['head_branch']])->whereNull('UPER_CUST_ID')->get();
				if (count($upPercent) == 0){
					$migrateTariff = false;
				}else{
			...
			if ($migrateTariff == true) {
				foreach ($upers as $uper) {
					...
					foreach ($group_tariff as $grpTrf) {
						$grpTrf = (array)$grpTrf;
						$uperD = DB::connection('eng_ilcs')->table('TX_TEMP_TARIFF_DTL')->where('TEMP_HDR_ID',$uper['temp_hdr_id'])->where('group_tariff_id',$grpTrf['group_tariff_id'])->get();
