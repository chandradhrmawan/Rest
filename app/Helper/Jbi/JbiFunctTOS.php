...
class JbiFunctTOS {
	...
	public static function sendRequestBookingJBI($arr){
    	$in_array = [
    		'TX_HDR_CANCELLED',
    		'TX_HDR_REC',
    		'TX_HDR_DEL',
    	...
    	if (!in_array($arr['table'], $in_array)) {
	        ...
	        $res = JbiConnectedExternalApps::sendRequestToExtJsonMet($opt);
	        $res = static::decodeResultAftrSendToTosNPKS($res, 'repoPost');
			// Simpan ke TX_SERVICES error lit ini
			// JbiConnectedExternalApps::storeTxServices($json,json_decode($json,true)["repoPostRequest"]["esbBody"]["request"],$res["result"]["result"]);

    	}
        return ['sendRequestBookingJBI' => $res];
	}

	public static function getRealJBI($input){
		...
		$find = (array)$find;
		$dtlLoop = DB::connection('omuster_ilcs')->table($config['head_tab_detil'])->where($config['head_forigen'], $input['id'])->whereIn($config['DTL_FL_REAL'], $config['DTL_FL_REAL_S']);
		...
		if (count($dtlLoop) > 0) {
			$arr = static::getRealJsonJBI($find,$dtlLoop,$config);
			$res = JbiConnectedExternalApps::sendRequestToExtJsonMet($arr);
			$res = static::decodeResultAftrSendToTosNPKS($res, 'repoGet');
			if ($res['result']['count'] == 0) {
				$Success = false;
				$msg = 'realisasion not finish';
			}else{
				$his_cont = static::storeRealJBI($res['result']['result'],$find,$config,$input);
			}
		}
		$res['his_cont'] = $his_cont;
		$dtl = DB::connection('omuster_ilcs')->table($config['head_tab_detil'])->where($config['head_forigen'], $input['id']);
		if (!empty($config['DTL_IS_CANCEL'])) {
	...
	private static function storeReal($data,$hdr,$config,$input){
		$his_cont = [];
		foreach ($data as $listR) {
			...
			DB::connection('omuster_ilcs')->table($config['head_tab_detil'])->where($config['head_forigen'], $hdr[$config['head_primery']])->where($config['DTL_BL'], $listR['NO_CONTAINER'])->update($upSttDtl);
			...
			if (isset($real_value["nota_id"])) {
				...
				$cekTsCont = DB::connection('omuster_ilcs')->table('TS_CONTAINER')->where($findTsCont)->first();
				...
				$his_cont[] = JbiContHist::storeTsContAndTxHisCont($arrStoreTsContAndTxHisCont);
			}
		}
		return $his_cont;
	}

	public static function getRealJsonJBI($find,$dtlLoop,$config){
		$dtl = '';
		$arrdtl = [];
	...
	public static function storeGATI($listR,$hdr,$config,$input){
		...
		$cek = DB::connection('omuster_ilcs')->table('TX_GATEIN')->where($findGATI)->first();
		...
		if (empty($cek)) {
			DB::connection('omuster_ilcs')->table('TX_GATEIN')->insert($storeGATI);
		}else{
			DB::connection('omuster_ilcs')->table('TX_GATEIN')->where($findGATI)->update($storeGATI);
		}
	...
	public static function storeGATO($listR,$hdr,$config,$input){
		...
		$cek 		= DB::connection('omuster_ilcs')->table('TX_GATEOUT')->where($findGATO)->first();
		$datenow    = Carbon::now()->format('Y-m-d');
		$storeGATO  = [
			"gateout_cont" 			 	=> $listR['NO_CONTAINER'],
			"gateout_req_no" 		 	=> $listR['NO_REQUEST'],
		...
		if (empty($cek)) {
			DB::connection('omuster_ilcs')->table('TX_GATEOUT')->insert($storeGATO);
		} else {
			DB::connection('omuster_ilcs')->table('TX_GATEOUT')->where($findGATO)->update($storeGATO);
		}

		DB::connection('omuster_ilcs')->table($config['head_tab_detil'])->where($config['head_forigen'], $hdr[$config['head_primery']])->where($config['DTL_BL'], $listR['NO_CONTAINER'])->update([
	...
	public static function storeRealDateRec($listR,$hdr,$config,$input) {
		...
		$dataDetail 								= DB::connection('omuster_ilcs')->table('TX_DTL_REC_CARGO')->where($findDtlRecBrg)->update($updateVal);
		$dataDetail 								= DB::connection('omuster_ilcs')->table('TX_DTL_REC_CARGO')->where($findDtlRecBrg)->first();
		$qty 												= $dataDetail->rec_cargo_dtl_qty;
		$qtyReal 										= $dataDetail->rec_cargo_dtl_real_qty;

		$real 							= DB::connection('omuster_ilcs')->table('TX_REALISASI_CARGO')->where($findRealRecBrg)->get();
		if (empty($real)) {
			$insertReal 		  = DB::connection('omuster_ilcs')->table('TX_REALISASI_CARGO')->insert($storeRealisasi);
		} else {
			$updateReal 		  = DB::connection('omuster_ilcs')->table('TX_REALISASI_CARGO')->where($findRealRecBrg)->update($storeRealisasi);
		}

		if ($qty <= $qtyReal) {
			$updateFlReal 			= DB::connection('omuster_ilcs')->table('TX_DTL_REC_CARGO')->where($findDtlRecBrg)->update(["REC_CARGO_FL_REAL"=>$config["DTL_FL_REAL_V"]]);
	...
	public static function storeRealDateDel($listR,$hdr,$config,$input) {
		...
		$dataDetail 								= DB::connection('omuster_ilcs')->table('TX_DTL_DEL_CARGO')->where($findDtlDelBrg)->update($updateVal);
		$dataDetail 								= DB::connection('omuster_ilcs')->table('TX_DTL_DEL_CARGO')->where($findDtlDelBrg)->first();
		$qty 												= $dataDetail->del_cargo_dtl_qty;
		$qtyReal 										= $dataDetail->del_cargo_dtl_real_qty;

		$real 							= DB::connection('omuster_ilcs')->table('TX_REALISASI_CARGO')->where($findRealDelBrg)->get();
		if (empty($real)) {
			$insertReal 		  = DB::connection('omuster_ilcs')->table('TX_REALISASI_CARGO')->insert($storeRealisasi);
		} else {
			$updateReal 		  = DB::connection('omuster_ilcs')->table('TX_REALISASI_CARGO')->where($findDtlDelBrg)->update($storeRealisasi);
		}

		if ($qty <= $qtyReal) {
			$updateFlReal 			= DB::connection('omuster_ilcs')->table('TX_DTL_DEL_CARGO')->where($findDtlDelBrg)->update(["DEL_CARGO_FL_REAL"=>$config["DTL_FL_REAL_V"]]);
	...
	// store request data to tos
		private static function duplicateAndStoreToRec($head,$dtls,$arr){
			...
			foreach ($dtls as $dtl) {
				...
				},'; /* jangan ditambah spasi atau enter krn nanti dipakai buat fungsi substr */
			}

			// untuk meng_ilcshilangkan koma di akhir foreach
			$storeRecDtlFix = substr($storeRecDtl,0,-1);
		...
		private static function upOldDetilExt($oldReq,$itemNo,$config){
			...
			$hdr = DB::connection('omuster_ilcs')->table($config['head_table'])->where($config['head_no'], $oldReq)->first();
			$hdr = (array)$hdr;
			DB::connection('omuster_ilcs')->table($config['head_tab_detil'])->where($config['head_forigen'], $hdr[$config['head_primery']])->where($config['DTL_BL'], $itemNo)->update($upSttDtl);
		}

		private static function getHeadFromTmReff10($head,$arr){
			return DB::connection('omuster_ilcs')->table('TM_REFF')->where([
	          'reff_tr_id' => 5,
		...
		private static function buildJsonTX_HDR_CANCELLED($arr) {
	        $arrdetil = '';
	        $dtls = DB::connection('omuster_ilcs')->table('TX_DTL_CANCELLED')->where('cancl_hdr_id', $arr['id'])->get();
	        foreach ($dtls as $dtl) {
	          $dtl = (array)$dtl;
	          $arrdetil .= '{
	        ...
	        $head = DB::connection('omuster_ilcs')->table('TX_HDR_CANCELLED')->where('cancelled_id', $arr['id'])->first();
	        $head = (array)$head;
	        $nota = DB::connection('omuster_ilcs')->table('TX_HDR_NOTA')->where('nota_req_no', $head['cancelled_no'])->first();
	        $nota_no = null;
	        $nota_date = null;
	        $nota_paid_date = null;
	        if (!empty($nota)) {
		...
		private static function buildJsonTX_HDR_REC($arr) {
	        $arrdetil = '';
	        $dtls = DB::connection('omuster_ilcs')->table($arr['config']['head_tab_detil'])->where($arr['config']['head_forigen'], $arr['id'])->where($arr['config']['DTL_IS_ACTIVE'],'Y')->get();
					$head = DB::connection('omuster_ilcs')->table($arr['config']['head_table'])->where($arr['config']['head_primery'], $arr['id'])->first();
	        $head = (array)$head;
					foreach ($dtls as $dtl) {
	          $dtl = (array)$dtl;
	          $arrdetil .= '{
	        ...
	        $arrdetil = substr($arrdetil, 0,-1);
	        $nota = DB::connection('omuster_ilcs')->table('TX_HDR_NOTA')->where('nota_req_no', $head[$arr['config']['head_no']])->first();
	        $nota_no = null;
	        $nota_date = null;
	        $nota_paid_date = null;
	        if (!empty($nota)) {
		...
		private static function buildJsonTX_HDR_DEL($arr){
	        ...
	        $rec_dr = DB::connection('omuster_ilcs')->table('TM_REFF')->where([
	          'reff_tr_id' => 5,
	          'reff_id' => $head[$arr['config']['head_from']]
	        ])->first();
		...
		private static function buildJsonTX_HDR_STUFF($arr){
	        ...
	        $head = DB::connection('omuster_ilcs')->table($arr['config']['head_table'])->where($arr['config']['head_primery'], $arr['id'])->first();
	        $head = (array)$head;
					$contFromName = DB::connection('omuster_ilcs')
		...
		private static function buildJsonTX_HDR_FUMI($arr){
	        ...
	        foreach ($dtls as $dtl) {
	          $dtl = (array)$dtl;
		        $getCountCounter = DB::connection('omuster_ilcs')->table('TS_CONTAINER')->where('cont_no',$dtl[$arr['config']['DTL_BL']])->orderBy('cont_counter','desc')->first();
	          $arrdetil .= '{
		...
		private static function buildJsonTX_HDR_REC_CARGO($arr) {
					...
					$head = DB::connection('omuster_ilcs')->table($arr['config']['head_table'])->where($arr['config']['head_primery'], $arr['id'])->first();
					$head = (array)$head;
					$dtls = DB::connection('omuster_ilcs')->table($arr['config']['head_tab_detil'])->where($arr['config']['head_forigen'], $arr['id'])->get();
					foreach ($dtls as $dtl) {
						$dtl = (array)$dtl;
